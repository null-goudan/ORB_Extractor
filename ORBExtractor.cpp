...
namespace Goudan
{
                                  ...
                                  std::vector<cv::KeyPoint> &_keypoints, cv::OutputArray _desciptors)
    {
        ...
        {
            _desciptors.create(nkeypoints, 32, CV_8U);
            desciptors = _desciptors.getMat();
        }
        ...
        for (int level = 0; level < nlevels; ++level)
        {
            ...
            for (int i = 0; i < nRows; ++i)
            {
                ...
                for (int j = 0; j < nCols; ++j)
                {
                    ...
                    if (!vKeyCell.empty())
                    {
                        for (vector<cv::KeyPoint>::iterator vit = vKeyCell.begin(); vit != vKeyCell.end(); vit++)
                        {
                            (*vit).pt.x += j * wCell;
                            (*vit).pt.y += i * hCell;
                                                              ...
                                                              const int &minX, const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
    {
        ...
        while (!bFinish)
        {
            ...
            // 如果现在已经有超过所需要的特征点个数的特征点，或者所有节点都只包含一个特征点
            // 那就结束
